...
local AdminList = {"Haggie125","Player1"} --Players who can use commands
local ScriptBanList = {} --Players who won't be allowed to join the game (Username or ID)(UserIDs are better)
local Prefixes = {":","/","ok google ","okay google ","ok google, ","okay google, ","alexa ","alexa, ","hey siri ","hey siri, ","please ","sudo "} --Command prefixes

--Optional Trello banlist
--Follow the instructions in the TrelloAPI module script to set up your key/token
local TrelloBoardName = "SHS Banlist"
...
local BanBoardID = Trello:GetBoardID(TrelloBoardName)
local BannedUserList = Trello:GetListID(BanListName,BanBoardID)
...
local Commands = {
	...
	["unpunish"] = function(Caller,Token)
		if Token[2] then
			local List = GetCharList(Caller,Token[2])
			for i=1,#List do
...
--					local Part = List[i]:GetChildren() --THIS IS UNNECESSARY
--					for x=1,#Part do
--						if Part[x].ClassName == "Accessory" then
--							Part[x].Handle.Mesh.Scale = Vector3.new(
--								Part[x].Handle.Mesh.Scale.x * ScaleChange,
--								Part[x].Handle.Mesh.Scale.y * ScaleChange,
--								Part[x].Handle.Mesh.Scale.z * ScaleChange
--							)
--							if Part[x].Handle:FindFirstChild("HatAttachment") then
--								local HandleOffset = (Part[x].Handle.Position.y - List[i].Head.Position.y)/PrevScale
--								Part[x].Handle.Mesh.Offset = Vector3.new(
--									0,
--									(HandleOffset*(tonumber(Token[3])-1)),
--									0
--								)
--							end
--						end
--					end
	...
	["invisible"] = function(Caller,Token)
	...
	["clip"] = function(Caller,Token)
		...
		if Token[2] then
			local List = GetPlayerList(Caller,Token[2])
			for i=1,#List do
				local Backpack = List[i]:FindFirstChild("PlayerGui")
				if Backpack then
					local ClipScript = Backpack:FindFirstChild("NoClip")
					if ClipScript then
						ClipScript:destroy()
					end
				end
	...
	["hideguis"] = function(Caller,Token)
		if Token[2] then
			local List = GetPlayerList(Caller,Token[2])
			for i=1,#List do
	...
	["time"] = function(Caller,Token)
		if Token[2] then
			game.Lighting.TimeOfDay = Token[2]
		end
	end,
	["clean"] = function(Caller,Token)
		for i=1,#DebrisList do
			DebrisList[i]:Destroy()
		end
		DebrisList = {}
	end,
	["cmds"] = function(Caller,Token)
		...
		if IsAdmin and IsCommand then
			...
			for i=1,#SubMessage do
				...
				if #Token > 0 then
					...
					--Check substitutions
					if Subs[Token[1]] then
						Token[1] = Subs[Token[1]]
					end
					if Commands[Token[1]] then
						--Execute command
						Commands[Token[1]](Player,Token)
						--[[
					elseif Token[1]:sub(1,5) == "wait(" then
						--Wait a bit
						local Time = tonumber(Token[1]:sub(6,#Token[1]-1))
						wait(Time)--]]
					elseif Token[1]:sub(1,4) == "wait" then
						local Time
						if Token[2] then
							Time = tonumber(Token[2])
						elseif Token[1]:sub(5,5) == "(" then
							Time = tonumber(Token[1]:sub(6,#Token[1]-1))
						end
						wait(time)
					end
				end
...
if TrelloBoardName and TrelloBoardName ~= "" then
	while true do
		UpdateTrelloBanlist()
		wait(60)
	end
